...
# Perform the process multiple times to analyze the distribution of SIF values
...
for n_comp in n_components_list:
    pca = PCA(n_components=n_comp).fit(tau_centered)
    components = pca.components_
    explained_var = np.sum(pca.explained_variance_ratio_)
    print(components.shape)
    print(f"n_components={n_comp}: Explained variance = {explained_var:.2%}")
    pca_results[n_comp] = {
        "components": components,
        "explained_variance_ratio": pca.explained_variance_ratio_,
        "explained_variance_sum": explained_var,
        "scores": pca.transform(tau_centered)
    }

def make_reflectance_model(m, n, f_matrix, mu_matrix2, mu_0_matrix2, irradiance_value, pixel_index):
    def model(lam, *params):
        a = np.array(params[0:m+1])
        b = np.array(params[m+1:n+m+1])
        c = params[n+m+1]
        attenuation = np.dot(b, f_matrix)
...
def make_reflectance_model_A(n, f_matrix, mu_matrix2, mu_0_matrix2, irradiance_value, pixel_index):
    def model(lam, *params):
        b = np.array(params[0:n])
        c = params[n]
        attenuation = np.dot(b, f_matrix)
...
m_values = [1, 2, 3, 4]  
num_iterations = 2
SIF_distributions_per_n = {}
SIF_distribution_A_per_n = {}

for n_comp in n_components_list:
    f_matrix = pca_results[n_comp]["components"]
    n = n_comp
    print(n_comp, n)
    SIF_distributions_per_m = {m: [] for m in m_values}
    SIF_distribution_A = []
    ...
    for iteration in tqdm(range(num_iterations)):
            for m in m_values:
                ...
                for pixel_index, i in enumerate(scanline_nocloud2):
                    reflectance_observed = amazon.variables['Reflectance'][0, i, 223, ind].data
                    reflectance_model = make_reflectance_model(m, n, f_matrix, mu_matrix2, mu_0_matrix2, irradiance_value, pixel_index)
                    p0 = [0.5] * (m+1) + [0.5] * n + [1]
                    popt, _ = curve_fit(reflectance_model, wl[ind], reflectance_observed, p0=p0)
                    SIF_values_per_scanline.append(popt[-1])
                SIF_distributions_per_m[m].append(SIF_values_per_scanline)
            # Fixed albedo SIF retrieval
            SIF_values_per_scanline_A = []
            for pixel_index, i in enumerate(scanline_nocloud2):
                reflectance_observed = amazon.variables['Reflectance'][0, i, 223, ind].data
                p0 = [0.5] * n + [1]
                reflectance_model_A = make_reflectance_model_A(n, f_matrix, mu_matrix2, mu_0_matrix2, irradiance_value, pixel_index)
                popt, _ = curve_fit(reflectance_model_A, wl[ind], reflectance_observed, p0=p0)
                SIF_values_per_scanline_A.append(popt[-1])
            SIF_distribution_A.append(SIF_values_per_scanline_A)
            
    SIF_distributions_per_n[n_comp] = SIF_distributions_per_m
    SIF_distribution_A_per_n[n_comp] = SIF_distribution_A


# Calculate statistics for the distributions for each n_components
stats_per_n = {}

for n_comp in n_components_list:
    SIF_distributions_per_m = SIF_distributions_per_n[n_comp]
    SIF_distribution_A = SIF_distribution_A_per_n[n_comp]
    for m in m_values:
        print(f"m={m}, num_iterations={len(SIF_distributions_per_m[m])}")
        if len(SIF_distributions_per_m[m]) > 0:
            print(f"  shape of first entry: {np.array(SIF_distributions_per_m[m][0]).shape}")
    mean_SIF_per_m = {m: np.mean(SIF_distributions_per_m[m], axis=0) for m in m_values}
    std_SIF_per_m = {m: np.std(SIF_distributions_per_m[m], axis=0) for m in m_values}
    mean_SIF_A = np.mean(SIF_distribution_A, axis=0)
    std_SIF_A = np.std(SIF_distribution_A, axis=0)

    stats_per_n[n_comp] = {
        "mean_SIF_per_m": mean_SIF_per_m,
        "std_SIF_per_m": std_SIF_per_m,
        "mean_SIF_A": mean_SIF_A,
        "std_SIF_A": std_SIF_A
    }

    print(f"\nStatistics for n_components={n_comp}:")
    for m in m_values:
        print(f"  m={m}: Mean={np.mean(mean_SIF_per_m[m]):.4f}, Std={np.mean(std_SIF_per_m[m]):.4f}")
    print(f"  Fixed Albedo: Mean={np.mean(mean_SIF_A):.4f}, Std={np.mean(std_SIF_A):.4f}")

    
    output_dir = "PCA_SIF_distributions"
    os.makedirs(output_dir, exist_ok=True)
    
    SIF_distributions_per_m = SIF_distributions_per_n[n_comp]
    for m in m_values:
        arr = np.array(SIF_distributions_per_m[m]) 
        np.save(os.path.join(output_dir, f"SIF_distribution_m_{m}_n{n_comp}.npy"), arr)
        mean_all = np.mean(arr)
        np.save(os.path.join(output_dir, f"mean_SIF_overall_m_{m}_n{n_comp}.npy"), mean_all)


    SIF_distribution_A = np.array(SIF_distribution_A_per_n[n_comp]) 
    np.save(os.path.join(output_dir, f"SIF_distribution_fixed_albedo_n{n_comp}.npy"), SIF_distribution_A)
    mean_all_A = np.mean(SIF_distribution_A)
    np.save(os.path.join(output_dir, f"mean_SIF_overall_fixed_albedo_n{n_comp}.npy"), mean_all_A)


        
selected_n_components = [10, 20, 30, 40, 50]

for n_comp in selected_n_components:
        SIF_distributions_per_m = SIF_distributions_per_n[n_comp]
        SIF_distribution_A = SIF_distribution_A_per_n[n_comp]
        ...
        print(f"  Fixed Albedo: Mean={np.mean(mean_SIF_A):.4f}, Std={np.mean(std_SIF_A):.4f}")
            ...
            # Plotting
        num_iterations = len(SIF_distribution_A)
        x = np.arange(num_iterations)

        def plot_sif_iterations(sif_iter_array, mean_all, title, filename):
            mean_per_iter = np.mean(sif_iter_array, axis=1)
            plt.figure(figsize=(8, 4))
            plt.plot(x, mean_per_iter, label="Mean SIF per iteration", color='blue', alpha=0.7)
            plt.axhline(mean_all, color='red', linestyle='--', linewidth=1.5, label="Overall mean SIF")
            plt.fill_between(x, mean_per_iter, mean_all, color='blue', alpha=0.2)                
            plt.xlabel("Iteration")
            plt.ylabel("Mean SIF (over all pixels)")
            plt.title(title)
            plt.legend()
            plt.tight_layout()
            ...
            # Fixed albedo
        mean_all_A = np.mean(mean_SIF_A)
        plot_sif_iterations(np.array(SIF_distribution_A), mean_all_A,
                            f"Fixed Albedo (n={n_comp})",
                            f"PCA_SIF_distributions/mean_SIF_fixed_albedo_iterations_n{n_comp}.png")

            # For each m
        for m in m_values: 
            sif_iter_array = np.array(SIF_distributions_per_m[m])
            mean_all = np.mean(mean_SIF_per_m[m])
            plot_sif_iterations(sif_iter_array, mean_all,
                                f"m={m} (n={n_comp})",
                                f"PCA_SIF_distributions/mean_SIF_m_{m}_iterations_n{n_comp}.png")

            # Save the mean and standard deviation of SIF values
        output_dir = "PCA_SIF_distributions"
        os.makedirs(output_dir, exist_ok=True)
        for m in m_values:
            np.save(os.path.join(output_dir, f"mean_SIF_m_{m}_n{n_comp}.npy"), mean_SIF_per_m[m])
            np.save(os.path.join(output_dir, f"std_SIF_m_{m}_n{n_comp}.npy"), std_SIF_per_m[m])
        np.save(os.path.join(output_dir, f"mean_SIF_fixed_albedo_n{n_comp}.npy"), mean_SIF_A)
        np.save(os.path.join(output_dir, f"std_SIF_fixed_albedo_n{n_comp}.npy"), std_SIF_A)
